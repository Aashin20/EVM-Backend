...
def MSR_CU_DMM_PAGINATED(
    ...
    """
    Fetch CU,DMM data in MSR Format with cursor pagination and filters
    """
    ...
    with Database.get_session() as db:
        try:
            ...
            query = db.query(
                ...
                dmm_comp.id.label('dmm_id'),  # For cursor
            ...
            if cursor:
                try:
                    ...
                    if direction == "next":
                        query = query.filter(dmm_comp.id > cursor_id)
                    else:  # prev
                        query = query.filter(dmm_comp.id < cursor_id)
                except ValueError:
            ...
            # Order and limit
            if direction == "prev":
                query = query.order_by(dmm_comp.id.desc())
            else:
                query = query.order_by(dmm_comp.id.asc())
...
def MSR_BU_PAGINATED(
    ...
    """
    Fetch BU data in MSR Format with cursor pagination and filters
    """
        ...
        try:
            ...
            if cursor:
                try:
                    cursor_id = int(cursor)
                    if direction == "next":
                        query = query.filter(EVMComponent.id > cursor_id)
                    else:  # prev
                        query = query.filter(EVMComponent.id < cursor_id)
                except ValueError:
                    raise HTTPException(status_code=400, detail="Invalid cursor format")
            ...
            # Order and limit
            if direction == "prev":
                query = query.order_by(EVMComponent.id.desc())
            else:
                query = query.order_by(EVMComponent.id.asc())
